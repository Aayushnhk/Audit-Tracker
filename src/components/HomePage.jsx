...
function HomePage() {
  ...
  // Define colors for dark/light modes for the chart
  ...
  const statsColors = {
    light: {
      "Total Observations": { color: "text-indigo-600", bg: "bg-indigo-50", iconBg: "bg-indigo-600" },
      "Open": { color: "text-red-600", bg: "bg-red-50", iconBg: "bg-red-600" },
      "In Progress": { color: "text-yellow-600", bg: "bg-yellow-50", iconBg: "bg-yellow-600" },
      "Closed": { color: "text-green-600", bg: "bg-green-50", iconBg: "bg-green-600" },
    },
    dark: {
      "Total Observations": { color: "text-indigo-400", bg: "bg-gray-800", iconBg: "bg-indigo-700" },
      "Open": { color: "text-red-600", bg: "bg-gray-800", iconBg: "bg-red-700" },
      "In Progress": { color: "text-yellow-600", bg: "bg-gray-800", iconBg: "bg-yellow-700" },
      "Closed": { color: "text-green-600", bg: "bg-gray-800", iconBg: "bg-green-700" },
    }
  };
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-gray-50 to-gray-100'} p-6 transition-colors duration-200`}>
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'} mb-1`}>
            Audit Tracker
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat) => (
            <div
              key={stat.name}
              className={`${darkMode ? statsColors.dark[stat.name].bg : statsColors.light[stat.name].bg} p-6 rounded-xl shadow-sm border ${darkMode ? 'border-gray-700' : 'border-gray-100'} hover:shadow-md transition-all duration-300 transform hover:-translate-y-1`}
            >
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`${darkMode ? statsColors.dark[stat.name].iconBg : statsColors.light[stat.name].iconBg} p-2 rounded-lg`}
                >
                  {
                    {
                      "Total Observations": (
              ...
              <p className={`${darkMode ? statsColors.dark[stat.name].color : statsColors.light[stat.name].color} text-4xl font-bold mt-4 mb-2`}>
                {stat.value}
              </p>
              {stat.name !== "Total Observations" && (
                <div className="relative pt-1">
                  ...
                  <div className={`overflow-hidden h-2 mb-4 text-xs flex rounded ${darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                    <div
                      className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center ${darkMode ? statsColors.dark[stat.name].iconBg : statsColors.light[stat.name].iconBg}`}
                      style={{
                        width: `${Math.round(
                          (stat.value / Math.max(1, observations.length)) * 100
                        )}%`,
                  ...
                  <defs>
                    {statusOptions.map((status) => (
                      <linearGradient
                        ...
                        y2="1"
                      >
                        <stop offset="5%" stopColor={darkMode ? chartStatusColors.dark[status] : chartStatusColors.light[status]} stopOpacity={0.8} />
                        <stop offset="95%" stopColor={darkMode ? chartStatusColors.dark[status] : chartStatusColors.light[status]} stopOpacity={1} />
                      </linearGradient>
                    ))}
                  </defs>
        ...
        <div className="mt-8 text-center">
          <Link
            to="/observations"
            className={`inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm ${darkMode ? 'text-white bg-indigo-700 hover:bg-indigo-600' : 'text-white bg-indigo-600 hover:bg-indigo-700'} focus:outline-none focus:ring-2 focus:ring-offset-2 ${darkMode ? 'focus:ring-offset-gray-900 focus:ring-indigo-500' : 'focus:ring-indigo-500'}`}
          >
